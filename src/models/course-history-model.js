...
function createCourseHistoryModel(db) {
  ...
  return {
    buildView(entries) {
      if (entries.length === 0) {
        return {
          message: 'No past course history records are available for this account yet.',
          state: 'empty',
          termGroups: []
        };
      }
      ...
      return {
        message: isPartial
          ? 'Some course history details are incomplete. Available records are shown below.'
          : 'Course history loaded successfully.',
...
module.exports = { createCourseHistoryModel };
