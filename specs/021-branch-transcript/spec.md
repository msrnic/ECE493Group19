...
2. **Given** only part of the student's transcript is available, **When** the system displays the results, **Then** the system clearly indicates that some terms, grades, or transcript entries are missing or unavailable.
...
- **FR-003**: The system MUST present transcript entries grouped by academic term and MUST show, for each available entry, the academic term, course identifier, course title, credits, and final grade or equivalent result when available.
- **FR-004**: The system MUST ensure that only the authorized student can view that student's transcript.
- **FR-005**: If some transcript data is unavailable or incomplete, the system MUST display the available entries and clearly indicate that some information is missing or unavailable.
...
- **FR-007**: If the actor is not authorized to view the requested transcript, the system MUST deny access and MUST NOT display transcript data.
- **FR-008**: The system MUST record each denied transcript access attempt for audit or security review.
...
- **NFR-001**: Transcript views, including full, partial, empty, unavailable, and denied states, MUST support keyboard-only navigation and provide headings and messages in text that assistive technologies can announce.
...
- **NFR-004**: In normal operating conditions, at least 95% of transcript page loads MUST render the first state-appropriate user-visible content within 2 seconds after the transcript view is requested.
...
| `FR-005` to `FR-006` | User Story 2, `AT-UC21-02` |
...
- **SC-004**: In scenario-based validation, at least 95% of authorized students can reach the transcript page and receive its first state-appropriate result within 30 seconds of selecting `View my transcript`.
