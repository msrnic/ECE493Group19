...
const { createCourseHistoryModel } = require('../../src/models/course-history-model');

test('course history model lists seeded entries and builds a full view', () => {
  const context = createTestContext();

  try {
    const model = createCourseHistoryModel(context.db);
...
test('course history model builds partial and empty views', () => {
  const context = createTestContext();

  try {
    const model = createCourseHistoryModel(context.db);
    const partialAccountId = context.app.locals.services.accountModel.findByIdentifier('prereq.student@example.com').id;
    const partialView = model.buildView(model.listByAccountId(partialAccountId));
    assert.equal(partialView.state, 'partial');
    assert.match(partialView.missingDataMessage, /reconciled/);
