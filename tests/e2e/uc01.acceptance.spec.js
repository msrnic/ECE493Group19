...
test('AT-UC01-04 admins can change another users password without target verification fields', async ({ browser, page }) => {
  ...
  await expect(page).toHaveURL(/\/dashboard$/);
  await page.locator('#security-center').getByLabel('Choose a user').selectOption({ label: 'Reset userA password' });
  await page.locator('#security-center').getByRole('button', { name: 'Open' }).click();
